...
(i)~perform {\it data discovery}, i.e., quickly find data of interest from large numbers of tables;
(ii)~built  a {\it linkage graph}, i.e., find ways to connect data sets of interest;
(iii)~find desired data from disparate data stores;
(iv)~clean the desired data; and %as it is often cited that 20\% of a typical data source is incorrect or missing; and
(v)~perform these tasks iteratively using a workflow system since data scientists often iterate through these steps in different orders.
...
Figure~\ref{fig:arch}.  The {\it offline component} indexes and profiles datasets;  these profiles are stored 
in a linkage graph, which is used to process workflow queries online. 
...
%
Our current prototype leverages \srm{Leverages sounds like it is taking advantage of known PK-FK relationships;  do we mean "searches for" or "Involves finding"} primary key-foreign key relationships (inclusion dependencies) for this purpose.
...
As such, the process of building the linkage graph can be run dynamically; we discuss its details in Section~\ref{sec:stitching}.
