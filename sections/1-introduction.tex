...
large numbers of tables; (ii)~{\it link} relevant data sets; %Mourad:Well link
...
desired data; and %as it is often cited that 20\% of a typical data source is
incorrect or missing; and (v)~{\it iterate} through these tasks using a workflow
system, since data scientists often \notera{perform these tasks} %use these modules 
in different orders.
...
%incrementally.} 
%\mourad{We could say: 
\notera{Initially, all linkages that can be found
in linear time are built during the offline stage. As new data is added or
existing data changes, the graph is incrementally updated to reflect the changes
in these linkages.} All the other linkages, such as \pkfk relationships, will be
...
iterate over its components in whatever order they wish\notera{.} Moreover, they
