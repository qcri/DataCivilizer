...
%For example, a data scientist at Merck wants to build an drug-indication ontology. 
...
\srm{Graph building does not need to be done all at once:  as data changes and new tables are added, they can be scanned and added to the graph incrementally.} \mourad{We could say:
Initially, all linkages that can be found in linear time are built during the offline stage. As new data is added or existing data changes, the graph is incrementally updated to reflect the changes in these linkages.} All the other linkages, such as \pkfk relationships, will be built in the background or on-demand during the online stage. Our current prototype identifies \pkfk relationships, using inclusion dependencies. As such, the process of building the linkage graph can be run dynamically and on-demand; we discuss its details in Section~\ref{sec:stitching}.
