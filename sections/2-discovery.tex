...
InfoGather~\cite{DBLP:conf/sigmod/YakoutGCC12}. \srm{I don't understand the following sentence, vis-a-vis the Goods paper, which seems to be about supporting data discovery for a bunch of use cases, without one specific use-case in mind.}\mourad{I agree with Sam; after skimming through the paper, I see Goods as a serious competitor to DC, at least on the discovery side.}  Unlike these systems
...
\srm{I think a little more detail is needed here describing how the profiles built above are used to answer these queries.  As it is the two subsections don't seem well connected -- why are profiles needed?}
...
the amount of work that downstream components  within \dcv must perform. \srm{Any suggestions about how to do this?}
