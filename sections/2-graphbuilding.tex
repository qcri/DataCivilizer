...
the same table or compound key. We call hyper-node to multiple simple nodes that
are connected by a hyper-edge. Each simple edge represents a relationship
between two nodes or hyper-nodes in the graph. Note that in this way the model
can express relationships involving individual columns as well as groups of
them. This is necessary to capture relationships among tables, \eg \pkfk
