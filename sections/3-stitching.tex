...
%\mourad{Raul and Dong need to agree on the
%structure of the graph, especially if nodes are tables or columns.} 
where each node represents a column and hyperedges connect
several columns to express compound keys or tables.\mourad{Dong, this is not consistent, in a hypergraph there are only ``hyperedges'' and they connect two or more nodes; so belonging to the same table, compound keys, column similarity, etc. are all represented by hyperedges. So no need to say ``to express compound keys or tables'' except if you want to say ``to be able to express relationships involving more that two nodes such as compound keys ...''} 
...
\emph{table equivalence}\mourad{Dong, what does it mean?}. 
...
%This approach avoids $n^2$ operations 
