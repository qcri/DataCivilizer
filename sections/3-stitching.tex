...
The linkage graph is a hyper-graph where each node represents a column and edges
represent a relationship between nodes of the graph. Note the hyper-edges can
express relationships involving more than two columns, such as compound keys and
within the same table. This is useful to allow capturing relationships among
tables, \eg subsumption, and compound keys \eg PKFK between compound keys. 
...
\emph{foreign-key and primary-key}, and
...
(PK-FK), inclusion dependency, and structure similarity. 
...
search space for the heavy relationships. After the user chooses the data of
