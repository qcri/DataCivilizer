...
\srm{please check the following} A straightforward computation of such relationships requires a $O(n^2)$
...
(LSH)~\cite{DBLP:conf/compgeom/DatarIIM04} to yield sub-quadratic runtimes;
%This approach avoids $n^2$ operations 
we have found LSH to run quite well in practice, even for data sets up to several Terabytes.
...
search space for the heavy relationships. Note that the data discovery can use the heavy \srm{do we mean light?} relationships, whenever available, to identify interesting data. After the user chooses the data of
interest, the graph builder can construct the heavy relationships amongst this
data online.
