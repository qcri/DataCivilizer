...
\nan{Raul and Dong, please formally define the graph model.}
...
%\sibo{The definition of node appears in Section \ref{introduction}, but it will be better to recap the definition again.}

%The input to the graph builder is a set of profiles that are attached to its nodes. The graph builder computes the edges among pairs of nodes. In particular, it finds content and schema similarity relationships. 
Each edge is scored with a weight that represents the strength of the relationship: the particular meaning
depends on the edge semantics.\mourad{Give examples.}  Also, edges with a
...
%discovered by a user \nan{These tables should not be discovered by a user?} 
and adds these linkages to the graph for future use. 
Data stitching utilizes an implementation of inclusion dependency to find candidate PK-FK relationships and then refines the candidates using advanced techniques\mourad{A bit hand-waving}. However, as is well known, data in the wild is quite dirty, which makes discovering inclusion dependencies difficult.  Specifically, foreign keys may not match a primary key, because of errors in either the PK or the FK\mourad{We could consider other interesting cases; something like http://www.vldb.org/pvldb/vol8/p1358-he.pdf}. To tolerate errors, we extend traditional inclusion dependency by both key coverage and text similarity and propose an \emph{\eind} as discussed in Section~\ref{subsec:eind}.
