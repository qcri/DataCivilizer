...
\dong{In \dcv, we collect information about the errors in each data set. Specifically, we need an accuracy metric for each cell, namely an estimate for the probability of the cell to be erroneous. To this end, \dcv first applies automatic error detection tools and then uses a composite metric to accumulate detected errors. The detect errors are categorized as follows.

\stab
(i)~Outliers -- these are data values that are distant from the distribution of values in the same column. 

\stab
(ii)~Duplicates -- these are a bunch of tuples that refer to the same real-world entity. The mismatching values between these duplicate tuples can signify an error.
...
The cleanliness of a join path is measured by the average cleanliness of all the cells in the result. \dcv selects the join paths based on the criteria which linear combines the size and the cleanliness of their corresponding query results. Note that \dcv also uses schema mapping~\cite{DBLP:conf/cidr/StonebrakerBIBCZPX13} to cluster the join paths based on their semantics and ranks the join paths within each categories for users to choose.}
