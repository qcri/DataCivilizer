...
\srm{Studied where?  What BigDAWG estimates?}\dong{I think BigDAWG may have a query optimizer that estimates the query result size. And we can use that to estimate the view size.}
Estimating the view size has been extensively studied in the literature~\cite{DBLP:conf/sigmod/IoannidisP95} and we just utilize the \texttt{BigDAWG} estimates.
