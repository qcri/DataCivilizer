...
Sometimes the users may prefer the join path that yields largest view size, i.e., the number of rows in the view. For example, the user may want a view with 1,000 rows and 90\% cleanliness rather than the view with 10 rows and 98\% cleanliness. We can combine all the facets as the criteria for join path selection. Estimating the view size has been extensively studied in our community and we just utilize the existing works~\ref{add reference}\dong{To cite the paper Ihab mentioned in the memting}.
