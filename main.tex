...
\newcommand{\mourad}[1]{\textcolor{blue}{~Mourad: #1}}
...
\newcommand{\titledcv}{{Data Civilizer}\xspace}
...
\newcommand{\pkfk}{PK-FK\xspace}
\newcommand{\Pkfk}{PK-FK\xspace}

%\newcommand{\pkfk}{primary key-foreign key\xspace}
%\newcommand{\Pkfk}{Primary key-foreign key\xspace}
...
\author{\alignauthor \vspace{-1em}
Dong Deng$^{\infty}$~~~Raul Castro Fernandez$^{\infty}$~~~Ziawasch Abedjan$^{\ddag}$\\Sibo Wang$^{\dagger}$\titlenote{Work done while doing an internship at QCRI.}~~~Michael Stonebraker$^{\infty}$~~~
Ahmed Elmagarmid$^{\sharp}$\\\vspace{.1em}Ihab F. Ilyas$^\ell$~~~Samuel Madden$^{\infty}$~~~Mourad Ouzzani$^{\sharp}$~~~Nan Tang$^{\sharp}$\\\vspace{.1em}
\affaddr{$^{\infty}$~MIT CSAIL~~~$^{\ddag}$~TU Berlin~~~$^{\dagger}$~Nanyang Technological University\\$^\ell$~University of Waterloo}~~~$^{\sharp}$~Qatar Computing Research Institute, HBKU \\\vspace{.1em}
\email{\{dongdeng, raulcf, stonebraker, madden\}@csail.mit.edu~~abedjan@tu-berlin.de\\swang@ntu.edu.sg~ilyas@uwaterloo.ca~
\{aelmagarmid,mouzzani,ntang\}@hbku.org.qa}
}
...
\input{sections/2-graphbuilding}
\input{sections/3-discovery}
\input{sections/4-curation}
%\input{sections/5-cleanliness_estimation}
%\input{sections/6-enhanced_stitching}
\input{sections/7-updates}
\input{sections/8-workflow}
\input{sections/9-wild}
