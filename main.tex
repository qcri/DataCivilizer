...
Dong Deng$^{\infty}$~~~Raul Castro Fernandez$^{\infty}$~~~Ziawasch Abedjan$^{\ddag}$~~~Sibo Wang$^{\sharp}$\titlenote{Work done while doing an internship at QCRI.} \\
Ahmed Elmagarmid$^{\sharp}$~~~Ihab F. Ilyas$^\ell$~~~Samuel Madden$^{\infty}$~~~Mourad Ouzzani$^{\sharp}$~~~Nan Tang$^{\sharp}$\\\vspace{.25em}
\affaddr{$^{\infty}$~MIT CSAIL~~~$^{\ddag}$~Technische Universität Berlin~~~$^\ell$~University of Waterloo\\$^{\sharp}$~Qatar Computing Research Institute, HBKU} \\\vspace{.1em}
\email{\{dongdeng, raulcf, madden\}@csail.mit.edu~~abedjan@tu-berlin.de\\ilyas@uwaterloo.ca~~
\{swang, aelmagarmid, mouzzani, ntang\}@qf.org.qa}
}
